...
import warnings
# get_torch_device import removed (iter22): was only used by the
# module-level device = get_torch_device() line that we patched out.

# iter22: module-level get_torch_device() removed -- caused ComfyUI to silently
# drop RIFE_VFI from NODE_CLASS_MAPPINGS when torch.cuda was not yet initialised
# at import time. All usages in warp() replaced with tenFlow.device (the tensor
# actual device, already known at call time).
backwarp_tenGrid = {}
