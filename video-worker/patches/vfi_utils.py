...
# iter22: module-level DEVICE = get_torch_device() removed -- this call executed
# at import time (when ComfyUI scans custom_nodes) and raised an exception on GPU
# workers before CUDA was fully initialised. ComfyUI catches that exception in its
# node-registration try/except and silently drops RIFE_VFI from NODE_CLASS_MAPPINGS.
# Replaced with a lazy getter that is called only during actual inference, when
# the GPU is guaranteed to be ready.
def _get_device():
    """Lazy wrapper: returns the torch device at call time, not at import time."""
...
def _generic_frame_loop(
        ...
        if use_timestep:
            for middle_i in range(1, multiplier):
                ...
                middle_frame = return_middle_frame_function(
                    frame0.to(_get_device()),
                    frame1.to(_get_device()),
                    timestep,
                    *return_middle_frame_function_args
                ).detach().cpu().to(dtype=dtype)
                middle_frames_out.append(middle_frame)
        else:
            middle_frames = non_timestep_inference(frame0.to(_get_device()), frame1.to(_get_device()), multiplier - 1)
    ...
    def _run_batched_pairs(pair_list):
        ...
        f0_batch = torch.cat([p[0] for p in pair_list], dim=0).to(_get_device())  # [B,C,H,W]
        f1_batch = torch.cat([p[1] for p in pair_list], dim=0).to(_get_device())
